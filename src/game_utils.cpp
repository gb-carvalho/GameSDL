...
void UpdateEnemyPosition(Enemy* enemy, SDL_Rect player_rect) {
    ...
    // Calcula a dire��o em que o inimigo deve se mover
    ...
    // Verifica se a magnitude � maior que zero antes de normalizar
    if (magnitude > 20) {
        // Normaliza a dire��o e move o inimigo
...
void randomizeCardArray() {
    ...
    // Reserva espa�o para evitar realoca��es
    eligible_cards.reserve(MAX_SIZE);
    not_eligible_cards.reserve(MAX_SIZE);

    // Classifica as cartas em eleg�veis e n�o eleg�veis
    ...
    // Preenche com cartas de n�vel m�ximo caso necess�rio
    ...
    // Preenche random_card_array com cartas aleat�rias
    for (int i = 0; i < random_card_array.size(); i++) {
        ...
        // Remove a carta de eligible_cards para evitar repeti��es
